...
# I built this SHA3 wrapper class in 2023 and im very proud of it,
# so I will try to implement it in any way possible even if its not
# needed.
...
class SHA3:
    ...
    def generate_salt(size: int = 32, isHex: bool = True) -> Union[str, bytes]:
        ...
        """
        salt = os.urandom(size).hex() if isHex else os.urandom(size)
        return salt
