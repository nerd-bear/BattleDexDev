...
class RateLimiter:
    ...
    def wait(self):
        ...
        if len(self.calls) >= self.max_calls:
            ...
            if sleep_time > 0:
                print(
                    f"⏱️ Rate limit reached. Pausing for {sleep_time:.1f} seconds...")
...
def get_planespotters_image(registration: str) -> str:
    ...
    try:
        headers = {
            'User-Agent': 'DiscordBot/1.0 FlightTracker (Python requests)'}
...
def get_flight_data(flight_identifier: str, api_token: str = None) -> list[dict]:
    ...
    if not token:
        raise ValueError(
            "FR24_API_TOKEN is missing. Set it as an environment variable or pass it to the function.")
    ...
    try:
        with Client(api_token=token) as client:
            ...
            response = client.live.flight_positions.get_full(
                flights=[target_flight])
            flight_data = getattr(response, 'data', [])

            if not flight_data:
                global_limiter.wait()
                response = client.live.flight_positions.get_full(
                    callsigns=[target_flight])
                flight_data = getattr(response, 'data', [])
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch live flight info from Flightradar24.")
    parser.add_argument(
        "flight_number", help="The flight number or callsign to search for (e.g., DL2204)")
    ...
    try:
        ...
        for data in results:
            print("\n" + "=" * 60)
            print(
                f" ✈️  FLIGHT: {data['callsign']} | REG: {data['registration']}")
            print(f" 🛫 AIRCRAFT: {data['aircraft_model']}")
            print("=" * 60)
            print(f"  🗺️ Position : {data['latitude']}, {data['longitude']}")
            print(
                f"  🗻 Altitude : {data['altitude_ft']:,} ft ({data['vertical_speed_trend']})")
